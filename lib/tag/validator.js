 ...
 * @param {Boolean|null} show only summary
 */
function validateTags(tags, verbose, summary) {
    var total = 0;
    var warnings = 0;

    (tags || []).forEach(tag => {

        var out = "\n" + _.padRight(colors.yellow(tag.name), 50, ' ') +
        ...
        // http:// links
        //
        var nonHttpsLinks = _.filter(htmls, param => param.value.match(/http:\/\//m));
